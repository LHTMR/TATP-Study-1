...
def _run_trial(running, application=APPLICATION):
...
def test_the_measured_force_is_empty_rather_than_the_label(running):
    """CLAUDE.md: never substitute a plausible value. The set is unweighed (FOR_S A3.1)."""
    session, _, _ = running
    _run_trial(running)
    row = _rows(session, "pinprick")[0]
    assert row["force_nominal_mn"] == "260.0"
    assert row["force_measured_mn"] == ""
    assert row["force_applied_mn"] == ""
...
def test_the_experimenter_is_told_what_to_apply_and_never_the_rating(running):
    ...
    _run_trial(running)
    assert "5.46" in experimenter.instruction.text()
...
def test_an_unknown_filament_is_refused_rather_than_guessed(running):
    ...
    with pytest.raises(KeyError, match="9.99"):
        trial.start()
