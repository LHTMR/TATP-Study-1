...
| N5.13 | **`touchcal_compare.judgement` allows `equal`, and nothing can produce it.** The comparison screen offers two buttons and asks which felt stronger; there is no third option and, since 26 Aug 2026, no confirm step where one could be added. This predates the direct-press change — the text screen it replaced had no `equal` route either — but the change makes it permanent, so it is worth saying rather than discovering during analysis. Either the column's domain drops `equal`, or the screen gains a third option, which would need approved wording and a third button the remote does not have. Flagged for S rather than decided. |
