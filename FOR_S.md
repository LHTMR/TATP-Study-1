...
| A2.3 | **Two things, now that the participant sets their own noise level (SPEC.md 10.7).** (a) **Fix the system output volume at bring-up and record it.** dBFS says nothing about sound pressure until it is fixed, so `white_noise_max_dbfs` is a stop on the software, not yet a safe level — set the OS volume once, meter it, and never change it. (b) **The experimenter alert must still be inaudible to the participant** at the *lowest* level any participant settles on, which is the worst case; that one is a meter check. The masking-check wording is drafted in `config/text/participant_{sv,en}.yaml` under `audio_setup` for your review. | Real participant | L3     |
