...
| #    | What                                                                                                                                                                                                                                                                                                          | Gate             | Ref    |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | ------ |
| A2.1 | **Software pressure ceiling.** 200 kPa is in place as a working value below the 250 kPa hardware maximum, and is not an agreed limit. Give a number or confirm it stands.<br/><br/><br/>S: use 250 kPa                                                                                                        | Real participant | L1     |
...
| A2.3 | **Audio levels** — white noise, participant cue, experimenter alert, in dBFS. Needs a meter at the headphones: the cue audible over the noise, the alert not audible to the participant.<br/><br/>S: we can just get the participant to adjust it, starting low. include testing if they can hear the sleeve. | Real participant | L3     |
...
| A3.3 | **How long a block is expected to take**, per type — the preview prints it and the overlap check needs it, and without it the check says it could not run. *Spacing is settled (23 Aug 2026): the rekindle takes its own place in the sequence, the last block lands on the end of the intervention, and the first is delayed by the pause. `make preview` shows the result — 50, 60, 70, 80, 90, 100, rekindle 105–110, 115, 125, 135, 145, 155, 165.* Set `generate.settled: true` alongside the durations to close this. | Piloting         | §20.4         |
