...
| B5.3 | **One permission hole I did not close.** `conda run … python` can execute anything and the Makefile needs it. I narrowed it to the four forms the Makefile actually uses rather than allowing it wholesale, but it is not airtight and I would rather say so than imply otherwise. |
