...
| A2.3 | **Two meter checks at bring-up**, now that the participant sets their own noise level (SPEC.md 10.7). (a) **Fix the system output volume and record it.** dBFS says nothing about sound pressure until it is fixed, so `white_noise_max_dbfs` is a stop on the software, not yet a safe level — set the OS volume once, meter it, never change it. (b) **The experimenter alert must stay inaudible to the participant** at the *lowest* level any participant settles on, which is its worst case. Wording approved and earplugs confirmed stocked, 23 Aug 2026. | Real participant | L3     |
